...
import { BadgeCheck, KeyRound, Layers3, Mail, ShieldCheck, UserPlus, UserRound } from "lucide-react";
...
export default function Register() {
  ...
  return (
    <div className="relative h-full overflow-hidden bg-[#050816] px-4 py-4 text-slate-100 sm:py-6">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(16,185,129,0.16),transparent_30%),radial-gradient(circle_at_20%_20%,rgba(34,211,238,0.16),transparent_28%),radial-gradient(circle_at_80%_15%,rgba(59,130,246,0.16),transparent_24%),linear-gradient(180deg,#050816_0%,#070b18_100%)]" />
      ...
      <div className="absolute left-8 top-10 h-56 w-56 rounded-full bg-emerald-500/18 blur-3xl" />
      <div className="absolute bottom-8 right-10 h-64 w-64 rounded-full bg-cyan-500/18 blur-3xl" />

      <div className="relative mx-auto flex h-full w-full max-w-6xl items-center justify-center">
        <div className="grid w-full items-center gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        <section className="order-1 hidden space-y-8 rounded-4xl p-8 backdrop-blur-xl lg:block lg:p-10">
          <div className="space-y-5">
            <h1 className="max-w-xl text-4xl font-semibold leading-15 text-white md:text-5xl">
              创建一个属于你的地理建模决策智能体
            </h1>
            <p className="max-w-2xl text-sm leading-7 text-slate-300 md:text-base">
              注册后即可使用地理建模智能决策系统，享受智能驱动的建模效率、人机对话的便捷交互和资源共享的统一管理
            </p>
          </div>

          <div className="grid gap-4 sm:grid-cols-3">
            {[
              { icon: ShieldCheck, title: "智能驱动", text: "利用AI技术提升地理建模效率" },
        ...
        <section className="order-2 w-full rounded-2xl border border-blue-400/20 bg-[#0b1223]/80 p-6 shadow-[0_24px_100px_rgba(8,15,35,0.85)] backdrop-blur-xl md:p-8 lg:justify-self-end">
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-slate-400">注册后即可使用地理建模智能决策系统</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-linear-to-r from-blue-800 to-emerald-500 px-4 py-3 text-sm font-semibold text-white shadow-[0_14px_35px_rgba(16,185,129,0.24)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_18px_45px_rgba(34,211,238,0.25)]"
            >
              <UserPlus size={16} />
              创建账号
            </button>
          ...
          <p className="mt-6 text-center text-sm text-slate-400">
            已有账号？
            <Link to="/login" className="ml-1 font-semibold text-emerald-300 transition hover:text-emerald-200">
              去登录
            </Link>
          </p>
        </section>
