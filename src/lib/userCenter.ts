...
export const getFavoriteModels = async (): Promise<FavoriteModel[]> => {
  return tryGetServerArray<FavoriteModel>("/user-center/favorites/models", normalizeFavoriteModel, FAVORITE_MODELS_STORAGE_KEY);
};
...
export const toggleFavoriteModel = async (model: Pick<FavoriteModel, "name" | "description" | "source">): Promise<boolean> => {
  ...
  if (exists) {
    try {
      await requestJson(`${BACK_URL}/user-center/favorites/models/${toEncoded(model.name)}`, {
        method: "DELETE",
      });
    } catch {
  ...
  try {
    const created = await requestJson<any>(`${BACK_URL}/user-center/favorites/models`, {
      method: "POST",
...
export const getFavoriteData = async (): Promise<FavoriteData[]> => {
  return tryGetServerArray<FavoriteData>("/user-center/favorites/data", normalizeFavoriteData, FAVORITE_DATA_STORAGE_KEY);
};
...
export const toggleFavoriteData = async (data: Pick<FavoriteData, "name" | "source" | "fromModel">): Promise<boolean> => {
  ...
  if (exists) {
    try {
      await requestJson(`${BACK_URL}/user-center/favorites/data/${toEncoded(data.name)}`, {
        method: "DELETE",
      });
    } catch {
  ...
  try {
    const created = await requestJson<any>(`${BACK_URL}/user-center/favorites/data`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
...
export const getSimulationResults = async (): Promise<SimulationResultItem[]> => {
  return tryGetServerArray<SimulationResultItem>("/user-center/simulation-results", normalizeSimulationResult, SIMULATION_RESULTS_STORAGE_KEY);
};
...
export const addSimulationResult = async (result: Omit<SimulationResultItem, "id" | "source" | "createdAt">) => {
  ...
  try {
    const created = await requestJson<any>(`${BACK_URL}/user-center/simulation-results`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
